    ...
    <table class="table table-striped">
        <tbody>
            ...
            function displayTable($db, $claimantID, $tableName, $title, $displayFn) {
                ...
                echo newTableRow(newTableColumn("") . newTableColumn(""));
                echo newTableRow(newTableColumn(bold("<p id='$tableName'>$title:</p>"))
                ...
                foreach ($rows as $row) {
                    ...
                    echo newTableRowWithAttributes(newTableColumn("")
                        . newTableColumn("<button type=\"button\" "
                            ...
                            . "onClick=\"hideAncillaryInfo('$tableInput', '$tableName', "
                            . $row["ID"] . ")\" "
            ...
            /*
            // Display the claimant information associated with this case that
            // is stored in the Dependant table

            // Create the edit/save/cancel buttons
            echo newTableRow(newTableColumn("") . newTableColumn(""));
            echo newTableRow(newTableColumn(bold("<p id='Dependant'>Dependants:</p>"))
                . newTableColumn('<div class="btn-group btn-group-sm pull-right">
			                  <button type="button" class="btn btn-primary"
			                    title="Edit" id="editButton_DependantInput"
			                    onclick="editButton(\'DependantInput\', \'Dependant\', '
                    . 'saveAncillaryInfoToDB)">
		                      <span class="glyphicon glyphicon-pencil"
		                        id="editButtonImage_DependantInput">
	                        </span>
			                  </button>
			                  <button type="button" class="btn btn-primary"
			                    title="Cancel" style="display:none;"
			                    id="restoreButton_DependantInput"
			                    onclick="restoreButton(\'DependantInput\', \'Dependant\', '
                    . 'saveAncillaryInfoToDB)">
			                    <span class="glyphicon glyphicon-remove">
		                      </span>
			                  </button>
            ...
            // Retrieve all dependants associated with this claimant
            $dependants = $db->interact(
                "SELECT * FROM `Dependant` WHERE `PersonID` = :id",
                array('id' => $case["ClaimantID"])
            )->fetchAll(PDO::FETCH_ASSOC);

            // Display all of this claimant's dependants
            foreach ($dependants as $row) {
                echo newDataRow("Dependant", "Dependant", $row,
                    "DependantName");

                // Create the button for removing dependants via the UI
                echo newTableRowWithAttributes(newTableColumn("")
                    . newTableColumn("<button type=\"button\" "
                        . "class=\"btn btn-danger DependantInputButton\" "
                        . "onClick=\"hideAncillaryInfo('DependantInput', 'Dependant', "
                        . $row["ID"] . ")\" "
                        . "disabled=\"disabled\">Remove</button>"),
                    array("name" => "Dependant" . $row["ID"]));
            }

            // Create the button for adding dependants via the UI
            echo newTableRowWithAttributes(
                newTableColumn('<button type="button" class="btn btn-primary DependantInputButton" disabled="disabled"
			                    title="Add" id="addAncillaryInfo_DependantInput"
			                    onclick="addAncillaryInfo(\'DependantInput\', \'Dependant\'); saveButton(\'DisabilityInput\', \'Disability\', saveAncillaryInfoToDB);location.reload();"
                                >Add Another Dependant</button>')
                . newTableColumn(""), array("id" => "addButtonRow_Dependant"));
                */

            // Display the claimant information associated with this case that
            // is stored in the Disability table

            /*
            // Create the edit/save/cancel buttons
            echo newTableRow(newTableColumn("") . newTableColumn(""));
            echo newTableRow(newTableColumn(bold("<p id='Disability'>Disabilities:</p>"))
                . newTableColumn('<div class="btn-group btn-group-sm pull-right">
			                  <button type="button" class="btn btn-primary"
			                    title="Edit" id="editButton_DisabilityInput"
			                    onclick="editButton(\'DisabilityInput\', \'Disability\', '
                    . 'saveAncillaryInfoToDB)">
		                      <span class="glyphicon glyphicon-pencil"
		                        id="editButtonImage_DisabilityInput">
	                        </span>
			                  </button>
			                  <button type="button" class="btn btn-primary"
			                    title="Cancel" style="display:none;"
			                    id="restoreButton_DisabilityInput"
			                    onclick="restoreButton(\'DisabilityInput\', \'Disability\', '
                    . 'saveAncillaryInfoToDB)">
			                    <span class="glyphicon glyphicon-remove">
		                      </span>
			                  </button>
            ...
            // Retrieve all disabilities associated with this claimant
            $disabilities = $db->interact(
                "SELECT * FROM `Disability` WHERE `PersonID` = :id",
                array('id' => $case["ClaimantID"])
            )->fetchAll(PDO::FETCH_ASSOC);

            // Display all of this claimant's disabilities
            foreach ($disabilities as $row) {
                ...
                // Create the button for removing disabilities via the UI
                echo newTableRowWithAttributes(newTableColumn("")
                    . newTableColumn(
                        "<button type=\"button\" class=\"btn btn-danger DisabilityInputButton\" "
                        . "onClick=\"hideAncillaryInfo('DisabilityInput', 'Disability', "
                        . $row["ID"] . ")\" disabled=\"disabled\">Remove</button>"),
                    array("name" => "Disability" . $row["ID"]));
            }

            // Create the button for adding disabilities via the UI
            echo newTableRowWithAttributes(
                newTableColumn('<button type="button" class="btn btn-primary DisabilityInputButton" disabled="disabled"
			                    title="Add" id="addAncillaryInfo_DisabilityInput"
			                    onclick="addNewDataRow( \'Disability\');"
                                >Add Another Disability</button>')
                . newTableColumn(""), array("id" => "addButtonRow_Disability"));
            ...
            */
            ...
            // Display the claimant information associated with this case that
            // is stored in the ThirdPartyTreatment table

            // Create the edit/save/cancel buttons
            echo newTableRow(newTableColumn("") . newTableColumn(""));
            echo newTableRow(newTableColumn(bold("<p id='ThirdPartyTreatment'>Third Party Treatment:</p>"))
                . newTableColumn('<div class="btn-group btn-group-sm pull-right">
			                  <button type="button" class="btn btn-primary"
			                    title="Edit" id="editButton_ThirdPartyTreatmentInput"
			                    onclick="editButton(\'ThirdPartyTreatmentInput\', \'ThirdPartyTreatment\', '
                    . 'saveAncillaryInfoToDB)">
		                      <span class="glyphicon glyphicon-pencil"
		                        id="editButtonImage_ThirdPartyTreatmentInput">
	                        </span>
			                  </button>
			                  <button type="button" class="btn btn-primary"
			                    title="Cancel" style="display:none;"
			                    id="restoreButton_ThirdPartyTreatmentInput"
			                    onclick="restoreButton(\'ThirdPartyTreatmentInput\', \'ThirdPartyTreatment\', '
                    . 'saveAncillaryInfoToDB)">
			                    <span class="glyphicon glyphicon-remove">
		                      </span>
			                  </button>
            ...
            // Retrieve all ThirdPartyTreatments associated with this claimant
            $thirdPartyTreatments = $db->interact(
                "SELECT * FROM `ThirdPartyTreatment` WHERE `PersonID` = :id",
                array('id' => $case["ClaimantID"])
            )->fetchAll(PDO::FETCH_ASSOC);

            // Display all of this claimant's ThirdPartyTreatments
            foreach ($thirdPartyTreatments as $row) {
                ...
                // Create the button for removing ThirdPartyTreatments via the UI
                echo newTableRowWithAttributes(newTableColumn("")
                    . newTableColumn(
                        "<button type=\"button\" class=\"btn btn-danger ThirdPartyTreatmentInputButton\" "
                        . "onClick=\"hideAncillaryInfo('ThirdPartyTreatmentInput', 'ThirdPartyTreatment', "
                        . $row["ID"] . ")\" disabled=\"disabled\">Remove</button>"),
                    array("name" => "ThirdPartyTreatment" . $row["ID"]));
            }

            // Create the button for adding ThirdPartyTreatments via the UI
            echo newTableRowWithAttributes(
                newTableColumn('<button type="button" class="btn btn-primary ThirdPartyTreatmentInputButton" disabled="disabled"
			                    title="Add" id="addAncillaryInfo_ThirdPartyTreatmentInput"
			                    onclick="addNewDataRow(\'ThirdPartyTreatment\')"
                                >Add Another Third Party Treatment</button>')
                . newTableColumn(""), array("id" => "addButtonRow_ThirdPartyTreatment"));

            // Display the claimant information associated with this case that
            // is stored in the VATreatment table

            // Create the edit/save/cancel buttons
            echo newTableRow(newTableColumn("") . newTableColumn(""));
            echo newTableRow(newTableColumn(bold("<p id='VATreatment'>VA Treatment:</p>"))
                . newTableColumn('<div class="btn-group btn-group-sm pull-right">
			                  <button type="button" class="btn btn-primary"
			                    title="Edit" id="editButton_VATreatmentInput"
			                    onclick="editButton(\'VATreatmentInput\', \'VATreatment\', '
                    . 'saveAncillaryInfoToDB)">
		                      <span class="glyphicon glyphicon-pencil"
		                        id="editButtonImage_VATreatmentInput">
	                        </span>
			                  </button>
			                  <button type="button" class="btn btn-primary"
			                    title="Cancel" style="display:none;"
			                    id="restoreButton_VATreatmentInput"
			                    onclick="restoreButton(\'VATreatmentInput\',  \'VATreatment\', '
                    . 'saveAncillaryInfoToDB)">
			                    <span class="glyphicon glyphicon-remove">
		                      </span>
			                  </button>
            ...
            // Retrieve all VATreatments associated with this claimant
            $VATreatments = $db->interact(
                "SELECT * FROM `VATreatment` WHERE `PersonID` = :id",
                array('id' => $case["ClaimantID"])
            )->fetchAll(PDO::FETCH_ASSOC);

            // Display all of this claimant's VATreatments
            foreach ($VATreatments as $row) {
                echo newDataRow("VA Treatment Facility", "VATreatment", $row,
                    "TreatmentFacility");

                // Create the button for removing VATreatments via the UI
                echo newTableRowWithAttributes(newTableColumn("")
                    . newTableColumn("<button type=\"button\" "
                        . "class=\"btn btn-danger VATreatmentInputButton\" "
                        . "onClick=\"hideAncillaryInfo('VATreatmentInput', "
                        . "'VATreatment', " . $row["ID"] . ")\" "
                        . "disabled=\"disabled\">Remove</button>"),
                    array("name" => "VATreatment" . $row["ID"]));
            }

            // Create the button for adding VATreatments via the UI
            echo newTableRowWithAttributes(
                newTableColumn('<button type="button" class="btn btn-primary VATreatmentInputButton" disabled="disabled"
			                    title="Add" id="addAncillaryInfo_VATreatmentInput"
			                    onclick="addNewDataRow(\'VATreatment\');"
                                >Add Another VA Treatment</button>')
                . newTableColumn(""), array("id" => "addButtonRow_VATreatment"));

            // Display the claimant information associated with this case that
            // is stored in the MilitaryService table

            // Create the edit/save/cancel buttons
            echo newTableRow(newTableColumn("") . newTableColumn(""));
            echo newTableRow(newTableColumn(bold("<p id='MilitaryService'>Military Service:</p>"))
                . newTableColumn('<div class="btn-group btn-group-sm pull-right">
			                  <button type="button" class="btn btn-primary"
			                    title="Edit" id="editButton_MilitaryServiceInput"
			                    onclick="editButton(\'MilitaryServiceInput\', \'MilitaryService\', '
                    . 'saveAncillaryInfoToDB)">
		                      <span class="glyphicon glyphicon-pencil"
		                        id="editButtonImage_MilitaryServiceInput">
	                        </span>
			                  </button>
			                  <button type="button" class="btn btn-primary"
			                    title="Cancel" style="display:none;"
			                    id="restoreButton_MilitaryServiceInput"
			                    onclick="restoreButton(\'MilitaryServiceInput\', \'MilitaryService\', '
                    . 'saveAncillaryInfoToDB)">
			                    <span class="glyphicon glyphicon-remove">
		                      </span>
			                  </button>
            ...
            // Retrieve all MilitaryServices associated with this claimant
            $militaryService = $db->interact(
                "SELECT * FROM `MilitaryService` WHERE `PersonID` = :id",
                array('id' => $case["ClaimantID"])
            )->fetchAll(PDO::FETCH_ASSOC);

            // Display all MilitaryServices associated with this claimant
            foreach ($militaryService as $row) {
                ...
                // Create the button for removing MilitaryServices via the UI
                echo newTableRowWithAttributes(newTableColumn("")
                    . newTableColumn(
                        "<button type=\"button\" class=\"btn btn-danger MilitaryServiceInputButton\" "
                        . "onClick=\"hideAncillaryInfo('MilitaryServiceInput', 'MilitaryService', "
                        . $row["ID"] . ")\" disabled=\"disabled\">Remove</button>"),
                    array("name" => "MilitaryService" . $row["ID"]));
            }

            // Create the button for adding MilitaryServices via the UI
            echo newTableRowWithAttributes(
                newTableColumn('<button type="button" class="btn btn-primary MilitaryServiceInputButton" disabled="disabled"
			                    title="Add" id="addAncillaryInfo_MilitaryServiceInput"
			                    onclick="addNewDataRow(\'MilitaryService\');"
                                >Add Another Military Service</button>')
                . newTableColumn(""), array("id" => "addButtonRow_MilitaryService"));

            // Display the claimant information associated with this case that
            // is stored in the WartimeService table

            // Create the edit/save/cancel buttons
            echo newTableRow(newTableColumn("") . newTableColumn(""));
            echo newTableRow(newTableColumn(bold("<p id='WartimeService'>Wartime Service:</p>"))
                . newTableColumn('<div class="btn-group btn-group-sm pull-right">
			                  <button type="button" class="btn btn-primary"
			                    title="Edit" id="editButton_WartimeServiceInput"
			                    onclick="editButton(\'WartimeServiceInput\', \'WartimeService\', '
                    . 'saveAncillaryInfoToDB)">
		                      <span class="glyphicon glyphicon-pencil"
		                        id="editButtonImage_WartimeServiceInput">
	                        </span>
			                  </button>
			                  <button type="button" class="btn btn-primary"
			                    title="Cancel" style="display:none;"
			                    id="restoreButton_WartimeServiceInput"
			                    onclick="restoreButton(\'WartimeServiceInput\', \'WartimeService\', '
                    . 'saveAncillaryInfoToDB)">
			                    <span class="glyphicon glyphicon-remove">
		                      </span>
			                  </button>
            ...
            // Retrieve all WartimeServices associated with this claimant
            $wartimeService = $db->interact(
                "SELECT * FROM `WartimeService` WHERE `PersonID` = :id",
                array('id' => $case["ClaimantID"])
            )->fetchAll(PDO::FETCH_ASSOC);

            // Display all WartimeServices associated with this claimant
            foreach ($wartimeService as $row) {
                ...
                // Create the button for removing WartimeServices via the UI
                echo newTableRowWithAttributes(newTableColumn("")
                    . newTableColumn(
                        "<button type=\"button\" class=\"btn btn-danger WartimeServiceInputButton\" "
                        . "onClick=\"hideAncillaryInfo('WartimeServiceInput', 'WartimeService', "
                        . $row["ID"] . ")\" disabled=\"disabled\">Remove</button>"),
                    array("name" => "WartimeService" . $row["ID"]));
            }

            // Create the button for adding WartimeServices via the UI
            echo newTableRowWithAttributes(
                newTableColumn('<button type="button" class="btn btn-primary WartimeServiceInputButton" disabled="disabled"
			                    title="Add" id="addAncillaryInfo_WartimeServiceInput"
			                    onclick="addNewDataRow(\'WartimeService\');"
                                >Add Another Wartime Service</button>')
                . newTableColumn(""), array("id" => "addButtonRow_WartimeService"));
            ?>
        </tbody>
    </table>
</div>
    ...
    $.post("removeFromVACaseSystemDB.php", dataKV,function (data) {
        if (data.indexOf("ERROR!!!") > -1) {
            ...
            var dataRows =
                document.getElementsByName(tableName + id);
            var item;
            while ((item = dataRows.item(0)) != null) {
                item.remove();
            }
        }
    }).always($.unblockUI);
