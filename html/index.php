...
<table class='table'>
                            ...
                            <div class="form-group">
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        ...
                                        <?php
                                        // Populate the claimant dropdown that
                                        // allows the user to add an existing
                                        // claimant to a new case or associate a
                                        // new claimant with the case
                                        $claimants = $db->interact("SELECT * FROM `Person`", array()
                                        )->fetchAll(PDO::FETCH_ASSOC);

                                        // We will display the ID and name of
                                        // each claimant in the dropdown
                                        foreach ($claimants as $row) {
                                            $firstName = $row["FirstName"];
                                            $lastName = $row["LastName"];
                                            $displayedName;
                                            // If the first and/or last name of
                                            // a claimant is null, adjust the
                                            // displayed name so it doesn't
                                            // begin or end with a comma
                                            if ($firstName == null) {
                                                if ($lastName == null) {
                                            ...
                                            // Create the entry in the claimant
                                            // dropdown that corresponds to this
                                            // claimant
                                            echo "
    ...
    <script>
        ...
        /* Changes the text in the "Add a Claimant" dropdown to match the name
         * of the claimant that was selected in the dropdown and updates the
         * variable used to track the selected claimant.
         *
         * selectedClaimantID - the ID of the claimant that was selected in the
         *                      claimant dropdown
         * newDisplayText - the name of the claimant that was selected in the
         *                  claimant drodown and will be displayed as the
