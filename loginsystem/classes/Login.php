...
class Login
{
    ...
    public function __construct()
    {
        ...
        // if user has an active session on the server
            ...
            } elseif(isset($_POST["admin_user_change_password"])){
                $this->changeUserPassword($_POST["user_password_new"]);
            }
    ...
    {
        // if database connection opened
        if ($this->databaseConnection() && array_key_exists('user_id', $_SESSION)) {
    ...
    public function changeUserPassword($user_password_new) {
            // now it gets a little bit crazy: check if we have a constant HASH_COST_FACTOR defined (in config/hashing.php),
            // if so: put the value into $hash_cost_factor, if not, make $hash_cost_factor = null
            $hash_cost_factor = (defined('HASH_COST_FACTOR') ? HASH_COST_FACTOR : null);
